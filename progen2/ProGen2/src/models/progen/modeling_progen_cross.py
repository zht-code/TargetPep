...
class ProGenCrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask=None):
        ...
        # query_layer = self.query_proj(query).view(-1, query.size(-2), self.num_attention_heads, self.head_dim).transpose(1, 2)
        # key_layer = self.key_proj(key).view(-1, key.size(-2), self.num_attention_heads, self.head_dim).transpose(1, 2)
        # value_layer = self.value_proj(value).view(-1, value.size(-2), self.num_attention_heads, self.head_dim).transpose(1, 2)
