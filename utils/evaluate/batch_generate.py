...
def batch_generate_peptides(model, pdb_folder, output_file):
    ...
    peptides_data = {}
        # 获取 PDB 文件夹中的所有文件夹名称
    pdb_id = [name for name in os.listdir(pdb_folder) if os.path.isdir(os.path.join(pdb_folder, name))]
    for file in pdb_id:
        receptor_pdb_path = os.path.join(pdb_folder, f"{file}/receptor.pdb")
        peptide_pdb_path = os.path.join(pdb_folder, f"{file}/peptide.pdb")
        # protein_id = pdb_folder.split(os.sep)[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_file", type=str, default="/root/autodl-tmp/PP_generate_v1/data/output/peptides_final1.json", help="Output JSON file for generated peptides.")
    parser.add_argument("--model_path", type=str, default = '/root/autodl-fs/pp_esm3/best_model_epoch_25_loss_0.4112.pth', help="Path to the trained model.")
    args = parser.parse_args()

    model = load_model(args.model_path)
    model.eval()
    batch_generate_peptides(model, args.pdb_folder, args.output_file)
