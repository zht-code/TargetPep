...
#     output_json = '/root/autodl-tmp/PP_generate_v1/data/result/peptides_vina3.json'
#     receptor_dir = '/root/autodl-tmp/PP_generate_v1/data/PPDbench'
#     ligand_dir = '/root/autodl-tmp/PP_generate_v1/data/downloads/codesign_ppflow'
...
#             ligand_pdb_file = os.path.join(ligand_dir, f"*_{protein_id}_*/0000.pdb")
...
#                 data[protein_id]["properties"][0]["PPFlow Affinity (kcal/mol)"] = affinity
...
# def run_qvina_and_parse(path_to_single_folder, receptor_pdbqt, ligand_pdbqt, center_x, center_y, center_z):
#     """运行 QVina 并解析日志文件，获取最好的 affinity"""
#     output_pdbqt = os.path.join(path_to_single_folder, "ppflow_vina.pdbqt")
#     command = [
#         '/root/qvina/qvina-master/bin/qvina02',
#         '--receptor', receptor_pdbqt,
#         '--ligand', ligand_pdbqt,
...
#     coords = []
#     # 遍历蛋白中所有原子，寻找与多肽原子距离小于 threshold 的情况
...
# def main():
#     json_file = '/root/autodl-tmp/PP_generate_v1/data/output/new_Experimental_vina_RFdiffusion.json'
#     output_json = '/root/autodl-tmp/PP_generate_v1/data/output/new_Experimental_vina_ppflow.json'
...
#             # 保存过滤后的结构
#             filtered_receptor_file = os.path.join(target_folder, "ppflow_receptor_filtered.pdb")
#             save_filtered_structure(filtered_receptor_structure, filtered_receptor_file)
#             filtered_peptide_file = os.path.join(target_folder, "ppflow_peptide_filtered.pdb")
#             save_filtered_structure(filtered_ligand_structure, filtered_peptide_file)
            
#             # 将 PDB 转换为 PDBQT 文件
#             receptor_pdbqt = pdb_to_pdbqt(filtered_receptor_file, os.path.join(target_folder, "ppflow_receptor.pdbqt"), is_receptor=True)
#             ligand_pdbqt = pdb_to_pdbqt(filtered_peptide_file, os.path.join(target_folder, "ppflow_peptide.pdbqt"), is_receptor=False)
...
def run_hdock_and_parse(path_to_single_folder, receptor_file, ligand_file):
    ...
    output_dir = os.path.join(path_to_single_folder, "ppflow5_hdock1")
...
def main():
    # json_file = '/root/autodl-tmp/PP_generate_v1/data/output/Hdock_RFdiffusion_vina.json'
    json_file = '/root/autodl-tmp/PP_generate_v1/data/output/peptides_final1_physical.json'
    output_json = '/root/autodl-tmp/PP_generate_v1/data/SOTA/ppflow5_hock.json'
    receptor_dir = '/root/autodl-fs/PPDbench'
    ligand_dir = '/root/autodl-tmp/ppflow1'
    ...
    for protein_id in tqdm(data.keys(), desc="Processing proteins"):
        try:
            receptor_pdb_file = os.path.join(receptor_dir, f"{protein_id}/receptor.pdb")
            ligand_pdb_file = os.path.join(ligand_dir, f"{protein_id}/0005.pdb")
            ligand_files = glob.glob(ligand_pdb_file)
            if not os.path.exists(receptor_pdb_file):
                print(f"Receptor PDB file does not exist: {receptor_pdb_file}. Skipping.")
                continue
            ...
            if "properties" in data[protein_id]:
                data[protein_id]["properties"][0]["PPFlow5 Affinity (kcal/mol)"] = score
