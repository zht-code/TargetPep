...
# #         # print(f"Error converting {pdb_file} to PDBQT: {result.stderr}")
# #         raise RuntimeError(f"Conversion failed for {pdb_file}")
...
#     """运行 QVina 并解析日志文件，获取最好的 affinity"""
#     output_pdbqt = os.path.join(path_to_single_folder, "vina.pdbqt")
...
#     json_file = '/root/autodl-tmp/PP_generate_v1/data/result/peptides_vina2.json'
#     # json_file = '/root/autodl-tmp/PP_generate_v1/data/result/peptides_vina.json'
#     output_json = '/root/autodl-tmp/PP_generate_v1/data/result/peptides_vina2.json'
#     receptor_dir = '/root/autodl-tmp/PP_generate_v1/data/PPDbench'
#     ligand_dir = '/root/autodl-tmp/PP_generate_v1/data/downloads/result'
...
#             ligand_pdb_file = os.path.join(ligand_dir, f"{protein_id}_unrelaxed_rank_001*.pdb")
...
#             # 保存过滤后的结构
#             filtered_receptor_file = os.path.join(path_to_single_folder, "receptor_filtered.pdb")
#             save_filtered_structure(filtered_receptor_structure, filtered_receptor_file)
#             filtered_peptide_file = os.path.join(path_to_single_folder, "peptide_filtered.pdb")
#             save_filtered_structure(filtered_ligand_structure, filtered_peptide_file)
            
#             # 将PDB转换为PDBQT
#             receptor_pdbqt = pdb_to_pdbqt(filtered_receptor_file, os.path.join(path_to_single_folder, "receptor.pdbqt"), is_receptor=True)
#             ligand_pdbqt = pdb_to_pdbqt(filtered_peptide_file, os.path.join(path_to_single_folder, "peptide.pdbqt"), is_receptor=False)
            
#             # 运行Qvina并获取亲和力 
#             affinity = run_qvina_and_parse(path_to_single_folder, receptor_pdbqt, ligand_pdbqt, center_x, center_y, center_z)
#             print(f"Best affinity for {protein_id}: {affinity} kcal/mol")
            
#             # 将亲和力添加到 JSON 数据中
#             if "properties" in data[protein_id]:
#                 data[protein_id]["properties"][0]["Affinity (kcal/mol)"] = affinity
...
# def run_qvina_and_parse(path_to_single_folder, receptor_pdbqt, ligand_pdbqt, center_x, center_y, center_z):
#     """运行 QVina 并解析日志文件，获取最好的 affinity"""
#     output_pdbqt = os.path.join(path_to_single_folder, "generate_vina.pdbqt")
...
# def main():
#     # 配置文件路径
#     json_file = '/root/autodl-tmp/PP_generate_v1/data/output/new_Experimental_vina_ppflow.json'
#     output_json = '/root/autodl-tmp/PP_generate_v1/data/output/new_Experimental_vina_generate.json'
#     receptor_dir = '/root/autodl-tmp/PP_generate_v1/data/test'
#     ligand_dir = '/root/autodl-tmp/PP_generate_v1/data/downloads/new_generated_peptide'
...
#             # 假设多肽文件名遵循模式，如 "{protein_id}_unrelaxed_rank_001*.pdb"
#             ligand_pdb_file = os.path.join(ligand_dir, f"{protein_id}_unrelaxed_rank_001*.pdb")
#             ligand_files = glob.glob(ligand_pdb_file)
#             if not os.path.exists(receptor_pdb_file):
#                 print(f"Receptor PDB file does not exist: {receptor_pdb_file}. Skipping.")
...
#             # 保存过滤后的结构到 PDB 文件
#             filtered_receptor_file = os.path.join(target_folder, "generate_receptor_filtered.pdb")
#             save_filtered_structure(filtered_receptor_structure, filtered_receptor_file)
#             filtered_peptide_file = os.path.join(target_folder, "generate_peptide_filtered.pdb")
#             save_filtered_structure(filtered_ligand_structure, filtered_peptide_file)
            
#             # 将 PDB 转换为 PDBQT 文件
#             receptor_pdbqt = pdb_to_pdbqt(filtered_receptor_file, os.path.join(target_folder, "generate_receptor.pdbqt"), is_receptor=True)
#             ligand_pdbqt = pdb_to_pdbqt(filtered_peptide_file, os.path.join(target_folder, "generate_peptide.pdbqt"), is_receptor=False)
...
def run_hdock_and_parse(path_to_single_folder, receptor_file, ligand_file):
    ...
    # output_dir = os.path.join(path_to_single_folder, "final_full")
    output_dir = os.path.join(path_to_single_folder, "peptides5")
    ...
    command1 = [
        ...
        '/root/autodl-tmp/phdock/Hdock.out',
        # '/root/Hdock.out',
        'top10.pdb',
        '-nmax', '10',
        '-complex', '-models'
    ...
    # result_file = os.path.join(output_dir, "model_1.pdb")  # 假设输出结果是最佳模型
     # 例如移动生成的文件到新位置
    shutil.move('/root/autodl-tmp/phdock/Hdock.out', output_dir)  # 修改路径为实际需要的路径
    shutil.move('/root/autodl-tmp/phdock/model_1.pdb', output_dir)  # 修改路径为实际需要的路径
...
def main():
    # json_file = '/root/autodl-tmp/PP_generate_v1/data/output/peptides_final1_physical.json'
    json_file = '/root/autodl-tmp/PP_generate_v1/data/output/peptides_final1_hdock.json'
    output_json = '/root/autodl-tmp/PP_generate_v1/data/SOTA/peptides5_hdock.json'
    # json_file = '/root/autodl-tmp/PP_generate_v1/data/output/quvina_final_physical.json'
    # output_json = '/root/autodl-tmp/PP_generate_v1/data/output/quvina_final_hdock.json'
    # json_file = '/root/autodl-tmp/PP_generate_v1/data/output/new_Experimental.json'
    receptor_dir = '/root/autodl-fs/PPDbench'
    # ligand_dir = '/root/autodl-fs/peptides_final1'
    ligand_dir = '/root/autodl-tmp/peptides5'
    ...
    for protein_id in tqdm(data.keys(), desc="Processing proteins"):
        # if protein_id in ['4f1z','4gq6','4gxl','4gyw','4h4f','4hom','4htp','4iim','4j8s','4k0u']:
        try:
            receptor_pdb_file = os.path.join(receptor_dir, f"{protein_id}/receptor.pdb")
            # 假设多肽文件名遵循模式，如 "{protein_id}_unrelaxed_rank_001*.pdb"
            ligand_pdb_file = os.path.join(ligand_dir, f"{protein_id}.pdb")
            ligand_files = glob.glob(ligand_pdb_file)
            if not os.path.exists(receptor_pdb_file):
                print(f"Receptor PDB file does not exist: {receptor_pdb_file}. Skipping.")
                continue
            ...
            if "properties" in data[protein_id]:
                # data[protein_id]["properties"][0]["generate Affinity (kcal/mol)"] = score
                data[protein_id]["properties"][0]["peptides5 Affinity (kcal/mol)"] = score
